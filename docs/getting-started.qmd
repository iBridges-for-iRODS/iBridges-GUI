...
![](screenshots/start_ibridges.png)
...
![](screenshots/add_check_config.png)

If you already stored an `irods_environment.json` in the directory `~/.irods` (Windows `C:\Users\<username>\.irods`) then you can use the drop-down menu (little dowmward arrow) to load the cinfiguration.

If you do not have an iRODS configuration yet, click on the button `New Config`.

You should see now something like:
![](screenshots/add_new_config.png)

Click the `Check` button to find out whether you can make a connection to the iRODS server. If you get error messages, adjust the contents accordingly. If you are unsure, please contact your iRODS service proivder.
